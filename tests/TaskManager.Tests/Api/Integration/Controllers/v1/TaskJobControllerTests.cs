...
public class TaskJobControllerTests : IntegrationTestBase<Program>
{
    ...
    [Fact(DisplayName = "Should return status 'Unprocessable Entity' when create a task job invalid")]
    public async Task CreateTaskJob_CreateATaskJobInvalid_ShouldReturnStatusUnprocessableEntity()
    {
        //Arrange
        var command = new CreateTaskJobRequest()
        {
            Name = "Task test",
            Description = "Task test valid",
    ...
    {
        //Arrange
        var status = await GetFirstStatus();
    ...
    [Fact(DisplayName = "Should return status 'Unprocessable Entity' when update a task job invalid")]
    public async Task UpdateTaskJob_UpdateATaskJobInvalid_ShouldReturnStatusUnprocessableEntity()
    {
        //Arrange
        ...
        var command = new UpdateTaskJobRequest()
        {
            ...
            Description = "Task update test invalid",
            DeliveryDate = DateTime.UtcNow,
            EstimateHours = 2,
            StatusId = Guid.Empty,
        };
    ...
    private async Task<Status> GetFirstStatus()
    {
        ...
        var statusRepositorye = scope.ServiceProvider.GetRequiredService<IStatusRepository>();

        var status = await statusRepositorye.GetAllAsync();
