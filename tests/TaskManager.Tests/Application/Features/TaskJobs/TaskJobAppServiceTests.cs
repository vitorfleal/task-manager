...
{
    [Fact(DisplayName = "Should return invalid response when create a task job with status null")]
    public async Task CreateTaskJob_NullStatus_ShouldReturnInvalidResponse()
    {
        // arrange
        var (_, service) = TaskJobContextMock(false);
        ...
        response?.Notifications.Should().Contain(n => n.Description == "Status not found");
    }
    ...
    public async Task UpdateTaskJob_UpdateAValidTaskJob_ShouldReturnValidResponse()
    {
        ...
        var (_, service) = TaskJobContextMock(true, true, true, request);
    ...
    public async Task UpdateTaskJob_UpdateAInValidTaskJob_ShouldReturnInValidResponse()
    {
        ...
        var (_, service) = TaskJobContextMock(true, true, false, request);
    ...
    [Fact(DisplayName = "Should return invalid response when update a task job return status null")]
    public async Task UpdateTaskJob_UpdateAInValidTaskJobStatus_ShouldReturnInValidResponse()
    {
        // arrange
        var request = TaskJobRequestHelper.UpdateTaskJobRequest();

        var (_, service) = TaskJobContextMock(false, true, true, request);
    ...
    public async Task RemoveTaskJob_UpdateAValidTaskJob_ShouldReturnValidResponse()
    {
        ...
        var (_, service) = TaskJobContextMock(false, false, true);
        ...
        UpdateTaskJobRequest? request = null)
    {
        ...
        var status = TaskJobRequestHelper.NewStatus();
        ...
        if (setupMockStatus)
            statusRepositoryMock.Setup(s => s.GetByIdAsync(It.IsAny<Guid>())).ReturnsAsync(status);

        if (setupMockMapper)
        {
            var expectedReturn = new TaskJob(request.Name, request.Description, request.DeliveryDate, request.EstimateHours, request.StatusId);
            mapperMock.Setup(m => m.Map(It.IsAny<UpdateTaskJobRequest>(), It.IsAny<TaskJob>())).Returns(expectedReturn);
        }

        var taskJobService = new TaskJobAppService(unitOfWorkMock.Object, mapperMock.Object, taskJobRepositoryMock.Object, statusRepositoryMock.Object);
