...
public class TaskJobAppService : AppService, ITaskJobAppService
{
    ...
    {
        var taskJob = new TaskJob(request.Name, request.Description, request.DeliveryDate, request.EstimateHours, request.StatusId);
        ...
        try
        {
            ...
            if (status is null)
                return (Response.Invalid("TaskJob", "Status not found"), taskJob);
