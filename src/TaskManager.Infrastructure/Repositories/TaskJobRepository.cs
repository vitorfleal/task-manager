...
public class TaskJobRepository : ITaskJobRepository
{
    ...
    public async Task<IEnumerable<TaskJob>> GetAllAsync() => await _tasks.AsNoTracking().Include(x => x.Status).ToListAsync();

    public async Task<TaskJob?> GetByIdAsync(Guid id) => await _tasks.AsNoTracking().Include(x => x.Status).FirstOrDefaultAsync(x => x.Id == id);
